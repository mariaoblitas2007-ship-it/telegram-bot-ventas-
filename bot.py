# ============================================================
# YANABICITASA - COMPLETO EXTENDIDO FINAL
# Negocio = SOLO mensajes + 5 fotos | @YanaBiBot = CON botones
# ============================================================
...
# TEXTOS COMPLETOS
# ============================================================
SALUDO_NEGOCIO_PROMO = """Wenas mor por aquí solo veo lo gratis 🙈✨
...
GRATIS_TODO = SALUDO_NEGOCIO_PROMO
...
# ============================================================
# 5 FOTOS - DOS VERSIONES
# ============================================================
async def promo_negocio_sin_botones(m):
    # Chat de Negocio = SIN botones, solo 5 fotos + texto
    try:
...
# ============================================================
# BOTONES (solo para @YanaBiBot, el negocio no usa botones)
# ============================================================
...
async def handle_all(update, context):
    ...
    # ========== CHAT DE NEGOCIO = SOLO PROMO, SIN BOTONES, CON 5 FOTOS ==========
    if es_neg:
        ...
        if not USUARIOS[uid].get('flags',{}).get('saludo_negocio'):
            await promo_negocio_sin_botones(m)
    ...
    # ========== @YanaBiBot = VENTAS + GRATIS + CON BOTONES + RESPONDE A TODO ==========
    else:
        intent = detectar_intencion(raw, cap)
        ...
        if intent == "vistas500":
            ...
            await context.bot.send_message(ADMIN_ID, f"✅ DICE 500 BOT @{un} {uid}", reply_markup=teclado_admin(uid,un))
            USUARIOS[uid].setdefault('flags',{})['pausado']=True; guardar_datos(); return

        if intent == "pago":
            await m.reply_text(PAGO_MSG, reply_markup=get_volver())
        ...
        if intent == "risa":
            await m.reply_text(no_repite(uid,'risa',["jajaja mor 😏 revisa mi canal y verás que no soy fake","😏 entra a mi canal mor ahí estoy subiendo todo"]), reply_markup=get_volver()); guardar_datos(); return
        if intent == "promo":
            await promo_bot_con_botones(m); guardar_datos(); prog_follow(context,uid,m.chat.id); return
...
def main():
    ...
    print("Bot completo listo: Negocio=solo mensajes+5 fotos / Bot=con botones+volver - sin spam - precios con videitos")
