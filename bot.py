...
RECORDATORIO_CORTO = "Ya te dejé los pasitos arribita mor 👆 cuando llegues a 100 me mandas captura + videito 🥺"
...
async def start_cmd(u,c):
    ...
    await enviar_5_fotos(u.message)
    await u.message.reply_text(GRATIS_TEXTO, reply_markup=get_menu())
...
async def btn(u,c):
    ...
    if d=='volver': await q.edit_message_text("¿Qué quieres mor? :3", reply_markup=get_menu())
        ...
        await q.message.reply_text(GRATIS_TEXTO, reply_markup=get_volver())
...
async def handle_all(update, context):
    ...
    if es_neg:
        ...
        if not USUARIOS[uid].get('flags',{}).get('saludo_negocio'):
            ...
            USUARIOS[uid].setdefault('flags',{})['saludo_negocio']=True
            USUARIOS[uid]['ultimo_promo']=ahora; USUARIOS[uid]['contador_msg']=0; guardar_datos(); return
