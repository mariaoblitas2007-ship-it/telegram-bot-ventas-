# ============================================================
# YANABICITASA - FINAL COMPLETO CORREGIDO
# Negocio = solo promo | @YanaBiBot = ventas + gratis + volver
# Anti-spam + no repite + precios con cantidad de videitos
# ============================================================
...
¿Quieres tus videitos gratis? Haz la promo de vistas mor :3"""
...
FALLBACK_MSGS = [
    ...
    "Entonces que dices mor, le damos gratis con tu QR o comprita? :3",
...
def detectar_intencion(txt, cap=""):
    ...
    if any(x in t for x in ['ya cumpli','cumpli','ya lo hice','ya esta']): return "cumplido"
    if any(x in t for x in ['yape','plin','pago','comprobante','transfer']): return "pago"
    if any(x in t for x in ['gratis','grtis','promo']): return "promo"
...
async def bienvenida_promo(m):
    ...
    await m.reply_text(GRATIS_TODO, parse_mode='HTML', reply_markup=get_volver())
...
async def followup_job(c):
    ...
    try:
        await c.bot.send_message(c.job.data['chat_id'], no_repite(uid,'follow',FALLBACK_MSGS), reply_markup=get_menu())
...
async def start_cmd(u,c):
    await u.message.reply_text("Hola mor 🥵 elige:", reply_markup=get_menu())
...
async def btn(u,c):
    ...
    if d == 'volver':
        ...
        await q.edit_message_text(precio_por_pais(d), parse_mode='HTML', reply_markup=get_volver())
    elif d == 'gratis':
        try: await q.message.reply_media_group([InputMediaPhoto(open(f'fotitos{i}.JPG','rb')) for i in range(1,6)])
        except:
            try: await q.message.reply_media_group([InputMediaPhoto(open(f'fotitos{i}.jpg','rb')) for i in range(1,6)])
            except: pass
        await q.message.reply_text(GRATIS_TODO, parse_mode='HTML', reply_markup=get_volver())
...
async def handle_all(update, context):
    ...
    ahora = time.time()
    if ahora - USUARIOS[uid].get('ultimo',0) < 1.8: return
    ...
    if es_neg:
        if m.photo or m.video:
            ...
            if es_100 and not es_500:
                txt = f"📈 NEGOCIO - LLEGÓ A 100\n👤 @{un} ID:<code>{uid}</code>\n📝 Razón: Llegó a 100, presiona para enviar mensaje 500\n🔗 {link_chat}"
        ...
        if not USUARIOS[uid].get('flags',{}).get('saludo_negocio'):
            await m.reply_text(SALUDO_NEGOCIO_PROMO)
            await bienvenida_promo(m)
        ...
        if intent in ["vistas100","cumplido","promo"]:
            txt = f"📈 NEGOCIO PIDE GRATIS\n👤 @{un} ID:<code>{uid}</code>\n💬 {raw}\n🔗 {link_chat}\n👉 Botón para enviar mensaje 500"
            await context.bot.send_message(ADMIN_ID, txt, parse_mode='HTML', reply_markup=teclado_admin_100(uid,un))
            guardar_datos(); return
        else:
            await m.reply_text("Por aquí solo veo lo gratis mor 🙈\n" + GRATIS_TODO, parse_mode='HTML', reply_markup=get_volver())
            return

    # ========== @YanaBiBot = VENTAS + GRATIS + RESPONDE A TODO ==========
    else:
        intent = detectar_intencion(raw, cap)
        pais = detectar_intencion(raw) # dummy para no confundir
        pais_real = normalizar(raw)
        pais_detect = None
        if any(x in pais_real for x in ['peru','pe']): pais_detect='pe'
        elif any(x in pais_real for x in ['mexico','mx']): pais_detect='mx'
        elif any(x in pais_real for x in ['usa','eeuu','colombia','argentina','chile','otros','dolar']): pais_detect='usa'

        if uid in ESPERA_PAIS and pais_detect:
            await m.reply_text(precio_por_pais(pais_detect), parse_mode='HTML', reply_markup=get_volver())
            del ESPERA_PAIS[uid]; guardar_datos(); prog_follow(context,uid,m.chat.id); return

        if intent == "pago":
            await m.reply_text(CUMPLIDO_MSG, reply_markup=get_volver())
        ...
        if intent in ["comprar","videollamada"]:
            ...
            ESPERA_PAIS[uid]=True; guardar_datos(); return

        if intent == "promo":
            await bienvenida_promo(m); guardar_datos(); prog_follow(context,uid,m.chat.id); return

        if intent == "real":
            await m.reply_text(no_repite(uid,'real',REAL_MSGS), reply_markup=get_volver()); guardar_datos(); return
        if intent == "salir":
            await m.reply_text(no_repite(uid,'salir',SALIR_MSGS), reply_markup=get_volver()); guardar_datos(); return

        # Cualquier mensaje = muestra menú con volver (más fácil)
...
def main():
    ...
    print("Bot final corregido listo: negocio solo promo / bot ventas+gratis+volver - precios con cantidad")
