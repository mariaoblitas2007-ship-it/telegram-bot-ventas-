...
# OCR opcional - no rompe el bot si falla
try:
    import pytesseract
    ...
    HAS_OCR = False
    logger.warning("pytesseract no instalado - fotos sin OCR")
...
def es_precio(t): return any(p in normalizar(t) for p in ['precio','precios','costo','cuanto','cuánto','vale'])
...
async def analizar_foto(ctx, uid, user, fid):
    try:
        ...
        if any(k in txt for k in ['yape','plin','paypal','banco','clabe']):
            tipo="💰 PAGO"; PAGARON.add(uid); guardar_datos()
        elif any(k in txt for k in ['tiktok','story']): tipo="📸 PROMO"
        else: tipo="🔥 FOTO"
...
async def todo(upd, ctx):
    ...
    if es_neg:
        ...
        # Solo responde a comandos reales
        ...
        if any(k in txt for k in ['vistas','500']): await m.reply_text("Comenta #hormo 'miren mi story' 🥵"); return
        if 'gratis' in txt: await m.reply_text("1.Bio Tg:yanabicitasa 2.Subir QR 3.Comentar 4.Captura\nDi 'ya'"); return
...
async def btn(upd,ctx):
    ...
    elif q.data in ['pe','mx','usa']: await q.edit_message_text({'pe':PE_PRECIOS,'mx':MX_PRECIOS,'usa':USA_PRECIOS}[q.data])
...
async def nuevo(upd,ctx):
    for u in upd.chat_member.new_chat_members:
        if not u.is_bot:
            INVITADOS[upd.chat_member.from_user.id]=INVITADOS.get(upd.chat_member.from_user.id,0)+1; guardar_datos()
